...
export const transformDashboardData = (result) => {
  ...
  if(result?.length > 0) {
    result.forEach( ticket =>{
      ...
      //For chart1 TAT
      const timeDifference = calculateTimeDifferenceInHours(ticket.start_date, ticket.due_date);
      ...
      for (let category in categories) {
        categories[category] = ((categories[category] / totalTickets) * 100).toFixed(2);
      }
